...
Environment setup is always painful, if getting errors say that one version of python is used, if you want to use the python(in your virtual environment), please restart R session and try a new environment name again. Also after you have setup the virtual environment. Next time to run the package, the virtual environment already exists, hence you only have to initiate the virtual environment by py_initial("name_of _you_virtual_environment")(i.e. "numba4r" here). 
```{r set up environment, eval=FALSE, message=FALSE, warning=FALSE}
create_venv("numba4r") #can specify any python path 
```
```{r python initiate, eval=FALSE}
py_initial("numba4r")
```
...
```{r echo=TRUE, fig.height=8, fig.show='asis', fig.width=8, warning=FALSE}
...
Hence, for ths strong biomarker effect condition, the simulation suggests that 2-cutoff design is optimal, where the smallest subpopulation consists of 13.9\% of the full population, the larger one consists of nearly 40\%. Then the design seperats the full population into three populations with two nested subpopulations. The Type-I error rate for each population is allocated as below. 
